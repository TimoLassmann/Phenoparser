...
try(system2('gemini',args=" query --header -q 'select chrom, start, end, gene, impact, impact_severity,
              ...
              (cadd_scaled >= 15 OR impact_severity == \"HIGH\")
               ' VARGEMINI_DATABASEPATH --gt-filter \"gt_depths.VARPATIENT_ID > 0\" > raw_var_table_VARPATIENT_ID.tsv", stderr="geministderr.txt", env = SNGGEMINIBIN))
```
...
try(system2("phenoparser",args=" panel  --id VARPATIENT_ID --db VAROMIM_DATABASEPATH --out VARPATIENT_ID", stderr="phenoparsererr.txt", env=SNGPPBIN))
