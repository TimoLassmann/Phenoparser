...
***  Master Template 
     ...
     #+BEGIN_SRC R :tangle report_V2_template.Rmd :exports code
       ...
       ```{r joining... } 
            ...
            pheno <-pheno %>%  group_by(gene) %>% dplyr::summarise(OmimPanel=paste(unique(OmimPanel), collapse=";"),Inheritance=paste(unique(Inheritance), collapse = ";"),PhenolyzerPanel=max(PhenolyzerPanel),evidence = paste(unique(evidence), collapse = ";")) %>% arrange(desc(PhenolyzerPanel))
       ...
       ```{r Sorting,include=TRUE}
                ...
                df  <- df  %>% select(evidence,Rank, pos, GTS, gene, Severity ,cadd_scaled, PhenolyzerPanel,OmimPanel,Inheritance,CallQ, Depth,  RefD,AltD, everything())
       ...
       df <- add_column(df , Toggle = "<font size=\"+2\" color=\"green\">&oplus;</font>",.before = 1)
       button_list <- list(list(extend='colvisGroup', text="Show additional columns", show=c(15:20)),
                           list(extend='colvisGroup', text="Hide additional columns", hide=c(15:20))
         ...
         datatable(df,rownames=FALSE,extensions = 'Buttons',filter = 'top',escape = FALSE, 
         options = list(
         dom = 'Blfrtip',
         buttons =  button_list,
         pageLength=25,
             autoWidth = FALSE,
             columnDefs = list(
                        list(visible=FALSE, targets= c(1,15:20)),
