   ...
   #+BEGIN_SRC sh :tangle make_extended_phenolyzer_database.sh  :exports code :shebang "#!/usr/bin/env bash" :noweb yes
     ...
     EOF
               ...
               function make_phenotype_tables()
               {
                   ...
                   try perl ~/programs/phenolyzer/disease_annotation.pl ./tmp/$outname -f -p -ph -logistic -out ./tmp/$phenolyzeroutput/$SAMPLENAME -addon DB_DISGENET_GENE_DISEASE_SCORE,DB_GAD_GENE_DISEASE_SCORE -addon_weight 0.25
                   try phenoparser readphe  --id $SAMPLENAME --pheno ./tmp/$phenolyzeroutput/$SAMPLENAME.seed_gene_list  --db $OUT_DATABASE
     ...
     #+BEGIN_SRC R :tangle report_V2_template.Rmd :exports code
       ...
       ```{r Add OMIM Phenotype information to df , include=FALSE}
                ...
                if(info[1,1] != 0){
                  ...
                  omim_genes = select(omim, X4,X6,X5)
                  colnames(omim_genes) = c("OmimPanel","gene","Inheritance")
                  omim_genes = distinct(omim_genes, OmimPanel, gene, Inheritance)
                }else{
                  omim_genes = tibble(OmimPanel = character(), gene = character(),Inheritance = character( ))
                }
       ...
       ```{r Add Phenolyzer information to df , include=FALSE}
                ...
                if(info[1,1] != 0){
                  ...
                  hpo_genes = distinct(hpo_genes, PhenolyzerPanel, gene)
                   ...
                   hpo_genes  = tibble(PhenolyzerPanel = numeric(), gene = character()) 
                }
       ...
       ```{r joining... } 
            ...
            pheno <-pheno %>%  group_by(gene) %>% dplyr::summarise(OmimPanel=paste(unique(OmimPanel), collapse=";"),Inheritance=paste(unique(Inheritance), collapse = ";"),PhenolyzerPanel=max(PhenolyzerPanel)) %>% arrange(desc(PhenolyzerPanel))
       ...
       ```{r Sorting,include=TRUE}
                ...
                df <- arrange(df,desc(PhenolyzerPanel * 1000 +  str_length(OmimPanel)),OmimPanel ,desc(PhenolyzerPanel), desc(cadd_scaled))

                df <- add_column(df , Rank = 1:nrow(df),.before = 1)

                df  <- df  %>% select(Rank, pos, GTS, gene, Severity ,cadd_scaled, PhenolyzerPanel,OmimPanel,Inheritance,CallQ, Depth,  RefD,AltD, everything())
       ...
       ```{r, echo=FALSE,include=TRUE}
              ...
              button_list <- list(list(extend='colvisGroup', text="Show additional columns", show=c(13:18)),
                                    list(extend='colvisGroup', text="Hide additional columns", hide=c(13:18))
                                    )
                ...
                options = list(
                    ...
                    autoWidth = FALSE,columnDefs = list(
                        ...
                        list(visible=FALSE, targets=c(13:18))
                    )
                )
                ) %>%
                  formatStyle(
