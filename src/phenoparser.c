...
static void endElement(void *userData, const XML_Char *name)
{
        ...
        state->data = realloc(state->data, sizeof(char*)* (state->lines+1)  );
        state->data[state->lines] = malloc(sizeof(char) * (state->characters.size+1));
        snprintf(state->data[state->lines], state->characters.size+1,"%s", state->characters.memory);
        state->lines++;
